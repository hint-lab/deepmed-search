...
export type BoostedSearchSnippet = SearchSnippet & {
    ...
    finalScore: number;
    score: number; // Added score for compatibility with ranking display
    merged: string; // Added merged content for display
}
...
export interface ChatCompletionChunk {
    ...
    choices: Array<{
        ...
        delta: {
            ...
            content?: string;
            type?: 'text' | 'think' | 'json' | 'error';
            url?: string; // Added for streaming visited URLs
            annotations?: Array<URLAnnotation>;
        };
        logprobs: null;
        finish_reason: null | 'stop' | 'thinking_end' | 'error';
    }>;
    usage?: any; // Usage might appear in the last chunk
    visitedURLs?: string[];
    readURLs?: string[];
    numURLs?: number;
}
...
// Tracker Types
// Assuming these are in utils, update path if needed
...
// Interface definitions for Jina API (if still used directly)
...
// Add any other types needed from the original file 